...
class SqlUtilTest {
	...
	@Test
	void canonicalizeForComparison_removesSurfaceDifferences() {
		assertEquals(SqlUtil.canonicalizeForComparison("select id from users where name = 'a' limit 10"),
				SqlUtil.canonicalizeForComparison("SELECT   id\n  FROM users\nWHERE name = 'a'  LIMIT 10;"));
	}

	@Test
	void canonicalizeForComparison_keepsSemanticDifferences() {
		assertNotEquals(SqlUtil.canonicalizeForComparison("select id from users limit 10"),
				SqlUtil.canonicalizeForComparison("select id from users limit 11"));
		assertNotEquals(SqlUtil.canonicalizeForComparison("select id from users where name = 'a'"),
				SqlUtil.canonicalizeForComparison("select id from users where name = 'A'"));
	}

	@Test
	void canonicalizeForComparison_unparsableInputFallsBackToCollapsedText() {
		assertEquals("select id from", SqlUtil.canonicalizeForComparison("  select   id\n from  "));
		assertEquals("", SqlUtil.canonicalizeForComparison(null));
		assertEquals("", SqlUtil.canonicalizeForComparison("   "));
	}
