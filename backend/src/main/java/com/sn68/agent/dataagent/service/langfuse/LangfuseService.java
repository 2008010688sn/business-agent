 ...
 */
package com.sn68.agent.dataagent.service.langfuse;
...
public class LangfuseService {
	...
	private static final AttributeKey<String> ATTR_AGENT_ID = AttributeKey.stringKey("agent.agent_id");

	private static final AttributeKey<String> ATTR_THREAD_ID = AttributeKey.stringKey("agent.thread_id");

	private static final AttributeKey<Boolean> ATTR_HUMAN_FEEDBACK = AttributeKey
		.booleanKey("agent.human_feedback");
	...
	// 键名必须与配置中心一致：此前读的是 langfuse.enabled，而实际配置键是
	// spring.ai.agent.langfuse.enabled，于是本项恒为默认值 true，配置侧关不掉。
	public LangfuseService(@Qualifier("langfuseTracer") Tracer langfuseTracer,
			@Value("${spring.ai.agent.langfuse.enabled:false}") boolean enabled) {
