...
public class SessionTraceStore implements SpanExporter {
	...
	private final LinkedHashMap<TraceRequestKey, TraceView> traceBySessionAndRequest = new LinkedHashMap<>(32, 0.75f,
			true);
	...
	public CompletableResultCode shutdown() {
		synchronized (monitor) {
			...
			traceBySessionAndRequest.clear();
		}
		return CompletableResultCode.ofSuccess();
	}
	...
	public Optional<TraceView> getTrace(String sessionId, String runtimeRequestId) {
		if (!StringUtils.hasText(runtimeRequestId)) {
			return getLatestTrace(sessionId);
		}
		synchronized (monitor) {
			return Optional.ofNullable(traceBySessionAndRequest.get(new TraceRequestKey(sessionId, runtimeRequestId)));
		}
	}
	...
	private void captureSpan(SpanData span) {
		...
		if (StringUtils.hasText(assembly.sessionId)) {
			TraceView traceView = assembly.toTraceView();
			latestTraceBySessionId.put(assembly.sessionId, traceView);
			if (StringUtils.hasText(assembly.runtimeRequestId)) {
				traceBySessionAndRequest.put(new TraceRequestKey(assembly.sessionId, assembly.runtimeRequestId),
						traceView);
			}
			evictOldSessionTraces();
			evictOldRequestTraces();
		}
	...
	private void evictOldSessionTraces() {
		while (latestTraceBySessionId.size() > MAX_SESSION_TRACES) {
			...
			if (removed != null) {
				...
				removeRequestTraceByTraceId(removed.traceId());
			}
		}
	}

	private void evictOldRequestTraces() {
		while (traceBySessionAndRequest.size() > MAX_TRACE_ASSEMBLIES) {
			traceBySessionAndRequest.remove(traceBySessionAndRequest.keySet().iterator().next());
		}
	}
	...
	private void removeSessionTraceByTraceId(String traceId) {
		...
		removeRequestTraceByTraceId(traceId);
	}

	private void removeRequestTraceByTraceId(String traceId) {
		traceBySessionAndRequest.entrySet().removeIf(entry -> Objects.equals(entry.getValue().traceId(), traceId));
	}
	...
	private record TraceRequestKey(String sessionId, String runtimeRequestId) {
	}
