...
import com.sn68.agent.dataagent.bo.schema.*;
import com.sn68.agent.dataagent.connector.SqlExecutor;
...
import static com.sn68.agent.dataagent.util.ColumnTypeUtil.wrapType;

@Slf4j
/**
 * Oracle 方言的库表结构（DDL）读取实现。方言差异点：模式即用户（ALL_USERS），
 * 表/列注释取 ALL_TABLES / ALL_TAB_COMMENTS / ALL_COL_COMMENTS 系列视图，
 * 行数限制使用 ROWNUM 而非 LIMIT，对象名默认大写。
 */
@Service
public class OracleJdbcDdl extends AbstractJdbcDdl {
	...
	public List<String> sampleColumn(Connection connection, String schema, String table, String column) {
		...
		try {
			...
			// Sample values feed the NL2SQL prompt; losing them silently degrades answer quality.
			log.warn("Failed to sample column values, continuing without samples. schema={}, table={}, column={}",
