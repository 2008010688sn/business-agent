...
/**
 * 企业微信群机器人通知适配器（WECOM / WECOM_ROBOT）：通过群机器人 Webhook 推送消息。
 */
@Component
public class WeComRobotAdapter extends AbstractWebhookNotificationAdapter {

	public WeComRobotAdapter(WebClient.Builder webClientBuilder) {
		super(webClientBuilder);
	}

	@Override
	public boolean supports(String provider, String channelType) {
		return "WECOM".equalsIgnoreCase(provider) && "WECOM_ROBOT".equalsIgnoreCase(channelType);
	}
	...
	public NotificationAdapterResult send(AgentNotificationConnector connector, AgentNotificationTarget target,
		...
		Map<String, Object> body = "text".equalsIgnoreCase(messageType)
				...
				: Map.of("msgtype", "markdown", "markdown", Map.of("content", rendered.content()));
