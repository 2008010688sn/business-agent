...
/**
 * 微信小程序订阅消息适配器（WECHAT_MINI / WECHAT_MINI_SUBSCRIBE）：向已订阅用户的 openid 推送订阅消息。
 */
@Component
public class WechatMiniSubscribeAdapter extends AbstractWebhookNotificationAdapter {

	public WechatMiniSubscribeAdapter(WebClient.Builder webClientBuilder) {
		super(webClientBuilder);
	}

	@Override
	public boolean supports(String provider, String channelType) {
		return "WECHAT_MINI".equalsIgnoreCase(provider) && "WECHAT_MINI_SUBSCRIBE".equalsIgnoreCase(channelType);
	}
	...
	public NotificationAdapterResult send(AgentNotificationConnector connector, AgentNotificationTarget target,
		...
		body.put("touser", required(firstText(stringValue(targetConfig.get("openId")), stringValue(targetConfig.get("touser"))),
				"WeChat Mini Program openId is required."));
		body.put("template_id", required(firstText(template.getPlatformTemplateId(), stringValue(targetConfig.get("templateId")),
				stringValue(connectorConfig.get("templateId"))), "WeChat Mini Program template_id is required."));
		putIfText(body, "page", firstText(stringValue(rendered.variables().get("page")), stringValue(targetConfig.get("page"))));
		putIfText(body, "miniprogram_state",
				firstText(stringValue(targetConfig.get("miniprogramState")), stringValue(connectorConfig.get("miniprogramState"))));
		putIfText(body, "lang", firstText(stringValue(targetConfig.get("lang")), stringValue(connectorConfig.get("lang"))));
	...
	private String accessToken(Map<String, Object> connectorConfig) {
		...
		if (!StringUtils.hasText(appId) || !StringUtils.hasText(appSecret)) {
			throw CheckedException.badRequest("WeChat Mini Program appId/appSecret or accessToken is required.");
		}
		...
		if (!StringUtils.hasText(stringValue(response.get("access_token")))) {
			throw CheckedException.badRequest("WeChat Mini Program access token request failed: " + response.get("errmsg"));
		}
	...
	private Map<String, Object> wechatData(Map<String, Object> variables) {
		...
		variables.forEach((key, value) -> {
			if (!StringUtils.hasText(key) || key.startsWith("_") || "page".equals(key)) {
				return;
			}
