 ...
 */
package com.sn68.agent.dataagent.connector.impls.postgre;
...
/**
 * PostgreSQL 方言的库表结构（DDL）读取实现。方言差异点：库列表读 pg_database，schema 读
 * information_schema.schemata，表/列注释取 pg_catalog（obj_description/pg_description），
 * 列采样与整表扫描使用 LIMIT 语法。
 *
 * @author jiuhe
 * @since 2024/3/15
...
public class PostgreJdbcDdl extends AbstractJdbcDdl {
	...
	public List<String> sampleColumn(Connection connection, String schema, String table, String column) {
		...
		try {
			...
			// 取样失败只降级为「无样例值」，不中断 schema 探查：样例值只用于丰富 NL2SQL 提示词。
