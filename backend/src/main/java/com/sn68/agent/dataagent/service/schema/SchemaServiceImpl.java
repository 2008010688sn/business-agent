...
import com.sn68.agent.dataagent.dto.schema.ColumnDTO;
import com.sn68.agent.dataagent.dto.schema.SchemaDTO;
import com.sn68.agent.dataagent.dto.schema.TableDTO;
...
import static com.sn68.agent.dataagent.util.DocumentConverterUtil.convertColumnsToDocuments;
import static com.sn68.agent.dataagent.util.DocumentConverterUtil.convertTablesToDocuments;
...
public class SchemaServiceImpl implements SchemaService {
	...
	public SchemaServiceImpl(@Qualifier("dbOperationExecutor") ExecutorService dbOperationExecutor,
			AccessorFactory accessorFactory, TableMetadataService tableMetadataService, BatchingStrategy batchingStrategy,
	...
	@Override
	public void buildSchemaFromDocuments(String skillId, List<Document> currentColumnDocuments,
		...
		if (!missingTables.isEmpty() && datasourceId != null) {
			loadMissingTableDocuments(skillId, datasourceId, mutableTableDocuments, missingTables);
			loadMissingColDocForMissingTables(skillId, datasourceId, mutableColumnDocuments, missingTables);
		}
	...
	@Override
	public Boolean schema(Long datasourceId, SchemaInitReq schemaInitRequest) throws Exception {
		log.info("Starting schema initialization for datasource: {}", datasourceId);
		DbConfigBO config = schemaInitRequest.getDbConfig();
		Long skillId = schemaInitRequest.getSkillId();
		Assert.notNull(skillId, "skillId cannot be null");
		...
		try {
			...
			clearSchemaDataForDatasource(skillId, datasourceId);
			...
			// 转换为文档
			List<Document> columnDocs = convertColumnsToDocuments(skillId, datasourceId, tables);
			List<Document> tableDocs = convertTablesToDocuments(skillId, datasourceId, tables);

			// 存储文档
			log.info("Storing  columns and {} tables for datasource: {}", columnDocs.size(), tableDocs.size(),
					datasourceId);
			storeSchemaDocuments(skillId, datasourceId, columnDocs, tableDocs);
	...
	protected void storeSchemaDocuments(Long skillId, Long datasourceId, List<Document> columns,
		...
		List<List<Document>> documentBatches = new ArrayList<>(columnBatches.size() + tableBatches.size());
		documentBatches.addAll(columnBatches);
		documentBatches.addAll(tableBatches);
		if (!documentBatches.isEmpty()) {
			agentVectorStoreService.addSkillDocumentBatches(skillId.toString(), documentBatches);
		}
	...
	protected void clearSchemaDataForDatasource(Long skillId, Long datasourceId) throws Exception {
		// 检查是否有文档需要删除
	...
	@Override
	public List<Document> getTableDocumentsByDatasource(String skillId, Long datasourceId, String query) {
		Assert.hasText(skillId, "skillId不能为空");
	...
	private void loadMissingTableDocuments(String skillId, Long datasourceId, List<Document> tableDocuments,
			List<String> missingTableNames) {
		// 加载缺失的表文档
		List<Document> foundTableDocs = this.getTableDocuments(skillId, datasourceId, missingTableNames);
	...
	private void loadMissingColDocForMissingTables(String skillId, Long datasourceId, List<Document> curColDocs,
			List<String> missingTableNames) {
		// 加载缺失的列文档
		List<Document> foundColumnDocs = this.getColumnDocumentsByTableName(skillId, datasourceId, missingTableNames);
	...
	@Override
	public List<Document> getTableDocuments(String skillId, Long datasourceId, List<String> tableNames) {
		Assert.hasText(skillId, "SkillId不能为空。");
		Assert.notNull(datasourceId, "DatasourceId cannot be null.");
		if (tableNames.isEmpty())
			return Collections.emptyList();
		// 通过元数据过滤查找目标表
		Filter.Expression filterExpression = DynamicFilterService.buildFilterExpressionForSearchTables(skillId,
	...
	@Override
	public List<Document> getColumnDocumentsByTableName(String skillId, Long datasourceId, List<String> tableNames) {
		Assert.hasText(skillId, "SkillId不能为空。");
		Assert.notNull(datasourceId, "DatasourceId cannot be null.");
		if (tableNames.isEmpty()) {
		...
		Filter.Expression filterExpression = dynamicFilterService.buildFilterExpressionForSearchColumns(skillId,
				datasourceId, tableNames);
		if (filterExpression == null) {
			log.error("FilterExpression is null.This should not happen when tableNames is not Empty, ");
			return Collections.emptyList();
