...
const routeIconFallbacks: Record<string, string> = {
  'ai-agent': 'hugeicons:ai-brain-04',
  'ai-agent_workspace': 'hugeicons:bot',
  'ai-agent_catalog': 'hugeicons:database',
  'ai-agent_employees': 'hugeicons:user-multiple',
  'ai-agent_ops': 'hugeicons:analytics-01',
  'ai-agent_quality': 'hugeicons:analytics-01',
  'ai-agent_integration': 'hugeicons:tools',
  'ai-agent_agent': 'hugeicons:bot',
  'ai-agent_agents': 'hugeicons:bot',
  'ai-agent_digital-employee': 'hugeicons:user-multiple',
  'ai-agent_digital-employees': 'hugeicons:user-multiple',
  'ai-agent_authorizations': 'hugeicons:key-01'
};

/** hugeicons 集合中不存在，写入 IAM 后侧栏会空白 */
const missingHugeicons = new Set(['hugeicons:user-smile', 'hugeicons:key-star']);
...
function getGlobalMenuByBaseRoute(route: RouteLocationNormalizedLoaded | ElegantConstRoute) {
  ...
  const usableIcon = icon && !missingHugeicons.has(icon) ? icon : '';
  const menuIcon = usableIcon || routeIconFallbacks[routeName] || import.meta.env.VITE_MENU_ICON;
  ...
  const menu: App.Global.Menu = {
    ...
    icon: SvgIconVNode({ icon: menuIcon, localIcon, fontSize: iconFontSize || 20 })
  };
