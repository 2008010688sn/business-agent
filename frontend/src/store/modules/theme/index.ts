...
import { applyThemeStylePreset } from '@/theme/style-mode';
import {
...
export const useThemeStore = defineStore(SetupStoreId.Theme, () => {
  ...
  /** Theme style */
  const themeStyle = computed(() => settings.value.themeStyle || 'minimal');
   ...
   * @param style
   */
  function setThemeStyle(style: UnionKey.ThemeStyle) {
    applyThemeStylePreset(settings.value, style);
  ...
  function updateThemeColors(key: App.Theme.ThemeColorKey, color: string) {
    ...
    if (key === 'primary') {
      ...
      localStg.set('themeColorSource', 'custom');
    } else {
  ...
  scope.run(() => {
    ...
    watch(
      themeStyle,
      val => {
        toggleThemeStyle(val);
        localStg.set('themeStyle', val);
      },
      { immediate: true }
    );
