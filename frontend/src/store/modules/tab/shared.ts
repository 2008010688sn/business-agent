...
export function getTabByRoute(route: App.Global.TabRoute) {
  const { name, path, fullPath = path } = route;

  const currentRouteMeta = route.matched
    ? route.matched.find(r => r.name === route.name)?.meta || route.meta
    : route.meta;

  const { title, i18nKey, fixedIndexInTab } = currentRouteMeta;
