...
export const useTabStore = defineStore(SetupStoreId.Tab, () => {
  ...
  function addTab(route: App.Global.TabRoute, active = true) {
    ...
    if (route.query && route.query.tabTitle) {
      tab.label = (route.query.tabTitle as string) || tab.label;
      if (tab.label.includes('.')) {
        tab.label = $t(tab.label as any);
        tab.i18nKey = route.query.tabTitle as App.I18n.I18nKey;
      }
    }
    ...
    if (!isHomeTab) {
      if (!isTabInTabs(tab.id, tabs.value)) {
        ...
        const tabItem = tabs.value.find(el => el?.id === tab?.id);
        if (tabItem) {
          tabItem.label = tab.label;
          if (tabItem?.routePath !== tabItem?.fullPath) {
            tabItem.fullPath = tab.fullPath;
          }
        }
        // const tabItem = tabs.value.find(el => el?.id === tab?.id && el?.routePath !== el?.fullPath);
        // if (tabItem) {
        //   tabItem.fullPath = tab.fullPath;
        // }
      }
   ...
   * @param retainFixedTabs Whether to keep fixed tabs
   */
  async function clearTabs(excludes: string[] = [], retainFixedTabs = true) {
    const remainTabIds = [...(retainFixedTabs ? getFixedTabIds(tabs.value) : []), ...excludes];
