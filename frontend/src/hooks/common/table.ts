...
export function useTable<A extends UI.TableApiFn>(config: UI.NaiveTableConfig<A>) {
  ...
  const {
    ...
    transformer: res => {
      ...
      const currentTrs = Number(current);
      const sizeTrs = Number(size);
      const totalTrs = Number(total);
      // Ensure that the size is greater than 0, If it is less than 0, it will cause paging calculation errors.
      const pageSize = sizeTrs <= 0 ? 10 : sizeTrs;

      const recordsWithIndex = records.map((item, index) => {
        return {
          ...item,
          index: (currentTrs - 1) * pageSize + index + 1
        };
      });
  ...
  const mobilePagination = computed(() => {
    const mobileLayout = isMobile.value ? 'total, prev, pager, next, jumper' : 'total, prev, pager, next, sizes, jumper';
