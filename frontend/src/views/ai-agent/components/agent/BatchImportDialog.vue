    ...
    <el-tabs v-model="importTab">
      <el-tab-pane label="JSON输入" name="input" v-if="importMode === 'json'">
        <JsonObjectEditor
          ...
          @validity-change="importJsonValid = $event"
        />
      ...
      <el-tab-pane label="Excel上传" name="input" v-if="importMode === 'excel'">
        <div style="text-align: center; padding: 40px 0">
          ...
          >
            ...
            <template #tip>
              <div class="el-upload__tip">只能上传 xlsx/xls 文件，且不超过 50MB</div>
            </template>
          </el-upload>
...
<script lang="ts">
  ...
  import type { BatchImportResult } from '@/views/ai-agent/services/skillSemanticModel';
  ...
  export default defineComponent({
    ...
    setup(props, { emit }) {
      ...
      const importJsonValid = ref(true);
      ...
      const handleValidateJson = () => {
        if (!importJsonValid.value) {
          ElMessage.warning('请先修正 JSON 格式错误');
          return false;
        }
        return props.validateJson(importJsonText.value);
      };
      ...
      const handleFileChange = (file: UploadFile) => {
        ...
        if (rawFile && rawFile.size > MAX_UPLOAD_FILE_SIZE) {
          ElMessage.warning(`文件大小不能超过 ${MAX_UPLOAD_FILE_SIZE_MB}MB`);
          uploadedFile.value = null;
          uploadRef.value?.clearFiles();
          return;
        }
      ...
      const handleExcelImport = async () => {
        ...
        if (uploadedFile.value.size > MAX_UPLOAD_FILE_SIZE) {
          ElMessage.warning(`文件大小不能超过 ${MAX_UPLOAD_FILE_SIZE_MB}MB`);
          uploadedFile.value = null;
          uploadRef.value?.clearFiles();
          return;
        ...
        try {
          ...
          ElMessage.error(`Excel 导入失败：${(error as Error).message}`);
