...
package com.sn68.agent.framework.security.domain;
...
public class UserInfoDetails implements Serializable {
    ...
    @Schema(description = "用户类型（内部用户、外部用户）")
    ...
    @Schema(description = "租户所属国家")
    private String tenantCountry;

    @Schema(description = "租户导出数据加密")
    private Boolean tenantExportEncrypt;

    @Schema(description = "用户导出数据加密")
    private Boolean userExportEncrypt;

    /**
     * 数据权限是否已加载完成.
     */
    @Builder.Default
    @Schema(description = "数据权限是否已加载完成")
    private Boolean dataPermissionLoaded = Boolean.TRUE;
