...
package com.sn68.agent.framework.boot.log.configuration;
...
import com.sn68.agent.framework.boot.log.AccessLogInfo;
import com.sn68.agent.framework.boot.log.AccessLogProperties;
import com.sn68.agent.framework.boot.log.AccessLogUtil;
import com.sn68.agent.framework.boot.log.event.AccessLogEvent;
import com.sn68.agent.framework.boot.log.handler.AbstractLogHandler;
import com.sn68.agent.framework.commons.RegionUtils;
...
public class AccessLogAspect {
     ...
     */
    @Pointcut("execution(public * com.sn68.agent..*.*(..)) && @annotation(com.sn68.agent.framework.commons.annotation.log.AccessLog)")
     ...
     */
    private Optional<AccessLogInfo> getLogInfo() {
    ...
    public void handleReturn(JoinPoint joinPoint, Object ret) {
        safeExecute(ignore -> {
            ...
            Optional<AccessLogInfo> logO = getLogInfo();
            logO.ifPresent(log -> {
    ...
    private void populateLogDetails(AccessLog annotation, JoinPoint joinPoint) {
        ...
        if (request != null) {
            ...
            if (userAgent != null) {
                log.setEngine(userAgent.getEngine() != null ? userAgent.getEngine().getName() : null);
                log.setOs(userAgent.getOs() != null ? userAgent.getOs().getName() : null);
                log.setPlatform(userAgent.getPlatform() != null ? userAgent.getPlatform().getName() : null);
                log.setBrowser(userAgent.getBrowser() != null ? userAgent.getBrowser().getName() : null);
            }
        }
    ...
    private void publishEvent(AccessLogInfo log) {
        ...
        applicationEventPublisher.publishEvent(new AccessLogEvent(log));
    }
}
